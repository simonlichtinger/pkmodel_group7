...
def test_base_model():
    ...
    test_model = PKModel()
    test_model.create_model("central", 1)   # Testing for just the main initial compartment functionality
    ...
    with pytest.raises(AssertionError):     # Shouldn't work because only one compartment but the given list is of length 2
...
def test_complex_network(): # create a model with non-standard values, and parent, child and sibling
    from ..pkmodel import PKModel
    test_model = PKModel()
    test_model.create_model("main",1,dosing_time_constant=2,elimination_time_constant=2)
